...
# ---------------------------------------------------------------------------
# Quad4 element (4-node bilinear quadrilateral, Gmsh type 3)
#
# Reference element: (ξ, η) ∈ [-1,1]²
# Node numbering (Gmsh 1-based): corners CCW
#
#   4-------3
#   |       |
#   |       |
...
Map (ξ,η) to physical space for a 4-node bilinear quad. `nodes` may be a
Vector{Int} or SVector{4,Int}.
"""
...
Compute the unit normal and area element at (ξ,η) for a 4-node bilinear quad.
"""
...
# ---------------------------------------------------------------------------
# Line2 element (2-node linear line, Gmsh type 1)
#
# Reference element: ξ ∈ [-1, 1]
# Node ordering (Gmsh 1-based): 1 = left endpoint, 2 = right endpoint
#
...
#   N₁ = ½(1−ξ)   N₂ = ½(1+ξ)
#
# Normal convention matches Line3: 90° CCW rotation of the unit tangent in xy.
# ---------------------------------------------------------------------------

@inline function line2_shape(ξ::Float64)
    return SVector(0.5*(1.0 - ξ), 0.5*(1.0 + ξ)), SVector(-0.5, 0.5)
end
...
`nodes` is a length-2 index vector [left corner, right corner].
"""
...
Compute the in-plane unit normal and arc-length element at ξ for a Line2
element lying in the xy-plane (z = const). For a straight segment the result
is independent of ξ.
"""
